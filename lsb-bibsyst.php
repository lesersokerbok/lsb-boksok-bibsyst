<?php
/**
 * Plugin Name: LSB Boksøk: Bibsyst-integrasjon
 * Description: Denne pluginen tar seg av Boksøks integrasjon med Bibsyst.
 * Version: 1.0.0
 * Text Domain: lsb-boksok-bibsyst
 * Author: Lilly Labs
...
class LSB_Bibsyst_Integration {
	...
	function load_text_domain() {
		load_plugin_textdomain( 'lsb-boksok-bibsyst', false, plugin_basename( dirname( __FILE__ ) ) );
	}
    ...
    function dependencies_notice() {
		...
		echo '<p>' . esc_html__( 'LSB Bibsyst er avhengig av post-typen "lsb_book".', 'lsb-boksok-bibsyst' ) . '</p>';
		echo '<p>' . esc_html__( 'Deaktiver og aktiver på ny når "lsb_book" er på plass.', 'lsb-boksok-bibsyst' ) . '</p>';
		echo '</div>';
    }
